	...
	if ( IsPlayerCharacter(this) ) then
		...
		for i,slot in pairs(ARMORSLOTS) do
			...
			if ( item ) then
				...
				agiBonus = agiBonus + ( EquipmentStats.BaseArmorClass[armorClass][slot].AgiBonus or 0 )
			else
				armorType = "Natural"
				armorClass = "Cloth"
			end
...
function RecalculateDefense()
	...
	this:SetStatValue("Defense", math.floor( (defense + GetMobileMod(MobileMod.DefensePlus)) * GetMobileMod(MobileMod.DefenseTimes, 1) ))
end
