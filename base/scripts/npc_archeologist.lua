...
RegisterEventHandler(EventType.ClientTargetGameObjResponse, "sell", 
	function(target,user)
		...
		if price > 0 then
			...
			text="[$2002]"..price.."[-] Copper!"
				
			response[1] = {}
			response[1].text = "It's all yours.[F7F700](Receive "..price.." Copper)[-]"
...
RegisterEventHandler(EventType.ClientTargetGameObjResponse, "identify", 
	function(target,user)
		...
		if (target == nil or not target:IsValid()) then return end
			...
			if (GetPrice ~= 0) then
				sellTarget = target
				text = text .. "\n\nI'll buy it for [F7F700]"..GetPrice(target).."Copper.[-]"
				response[1] = {}
				response[1].text = "It's all yours.[F7F700](Receive "..GetPrice(target).." Copper)[-]"
				response[1].handle = "SellArtifact" 
			end
			response[2] = {}
