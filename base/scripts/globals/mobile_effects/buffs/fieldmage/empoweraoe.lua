...
MobileEffectLibrary.EmpowerAoE = 
{
	...
	OnEnterState = function(self,root,target,args)
		...
		local protectionEnabled = target:GetObjVar("KarmaProtectionEnabled")

		local nearbyMobiles = FindObjects(SearchMobileInRange(self.Radius))
		for i=1,#nearbyMobiles do
			local mobile = nearbyMobiles[i]
        	if ( mobile ~= self.ParentObj and (not protectionEnabled or not ValidCombatTarget(target, mobile, true)) ) then
