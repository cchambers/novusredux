	...
	if (IsDead(this)) then
		...
		if ( corpse ~= nil ) then
			if ( corpse:IsValid() ) then
				-- corpse was found, add marker to location
				table.insert(mapIcons,{Icon="location_corpse", Location=corpse:GetLoc(), Tooltip="Corpse"})
			else
				local corpseWorld = this:GetObjVar("CorpseWorld")
				-- if we are in the same world, but corpse is invalid, must be NewCelador
				if ( corpseWorld == ServerSettings.WorldName ) then
					local corpseLoc = this:GetObjVar("CorpseLoc")
					if ( corpseLoc ) then
						table.insert(mapIcons,{Icon="location_corpse", Location=corpseLoc, Tooltip="Corpse"})
					end
				else
					local entrance
					if ( this:HasObjVar("CorpseSewerSubregion") ) then
						local sewerSubregion = this:GetObjVar("CorpseSewerSubregion")
						entrance = FindObject(SearchObjVar("SewerSubregion", sewerSubregion))
					else
						-- corpse must be in a dungeon region, look for entrance to that dungeon
						local corpseAddress = this:GetObjVar("CorpseAddress")
						local entrances = FindObjects(SearchObjVar("RegionAddress", corpseAddress))
						for i=1,#entrances do
							-- prevent finding anything else with the objvar, like player made portals
							if ( string.find(entrances[i]:GetCreationTemplateId(), "teleporter") ) then
								entrance = entrances[i]
								break
							end
						end
					end
					if ( entrance and entrance:IsValid() ) then
						table.insert(mapIcons,{Icon="location_corpse", Location=entrance:GetLoc(), Tooltip="Corpse"})
					end
				end
			end
		end
...
RegisterEventHandler(EventType.ClientUserCommand,"map",
	function ()
		...
		local knownMaps = this:GetObjVar("SubMaps") or {}
		...
		elseif(mapWindowOpen) then
			...
			--Check if player has this map memorized
			local mapIsKnown = false
			for i=1,#knownMaps do
				if (knownMaps[i] == mapName) then
					mapIsKnown = true
				end
			end

			--If they don't, load the world map
			if (not mapIsKnown) then
				mapName = ServerSettings.WorldName
			end
