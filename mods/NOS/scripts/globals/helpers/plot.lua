...
-- list of templates that don't allow moving into each other
local SolidObjects = {
    garden_raised_bed_small = true,
    garden_raised_bed_medium = true,
    garden_raised_bed_large = true,
    garden_plant_pot = true,
}
...
Plot.IsOwner = function(playerObj, controller)
    ...
    if ( IsGod(playerObj) and not TestMortal(playerObj) ) then return true end
...
Plot.GetUserPlots = function(userId)
    ...
    -- KH TODO: The structure needs to be updated to a key in the global record that can be reassigned
    local plots = GlobalVarRead("User.Plots."..userId) or {}
    -- KH Temp hack to solve TODO above for now
    local p = {}
    for i=1,20 do
        if ( plots[i] ~= nil ) then
            p[#p+1] = plots[i]
...
--- IsPassable function that skips plot objects
-- @param loc
-- @return true/false
Plot.IsPassable = function(loc)
    local passable = IsPassable(loc)
    if not( passable ) then
        collisionInfo = GetCollisionInfoAtLoc(loc) or {}
        local any
        for i=1,#collisionInfo do
            any = (type(collisionInfo[i]) == "userdata") -- static collision
            if not( any ) then
                local obj = GameObj(tonumber(StringSplit(collisionInfo[i], " ")[3]))
...
Plot.ValidateBound = function(playerObj, bound, size, ignoreLoc)
    ...
    for i=1,#pointsToCheck do
        ...
        if not( Plot.IsPassable(loc) ) then
...
Plot.GetInfoRemote = function(playerObj, controller, cb, id)
    ...
    if ( not controller ) then
        LuaDebugCallStack("[Plot.GetInfoRemote] controller not provided.")
        return
    end
...
Plot.TaxPayment = function(controller, playerObj, amount)
    ...
    -- update detailed ledger
    Plot.UpdateDetailLedger(controller, amount, playerObj)

    return true
end

Plot.UpdateDetailLedger = function(controller, amount, player, note)
    if not( controller ) then
        LuaDebugCallStack("[Plot.UpdateDetailLedger] controller not provided.")
        return
    end
    if not( amount ) then
        LuaDebugCallStack("[Plot.UpdateDetailLedger] amount not provided.")
        return
    end
    if not( player ) then player = GameObj(0) end

    local detail = controller:GetObjVar("PlotTaxLedgerDetailed") or {}
    table.insert(detail, 1, {amount, DateTime.UtcNow, player, note})
    while(#detail > ServerSettings.Plot.DetailedLedgerMaxLength) do
        table.remove(detail)
    end
    controller:SetObjVar("PlotTaxLedgerDetailed", detail)
end
...
Plot.Tax = function(controller)
    ...
    -- update detailed ledger
    Plot.UpdateDetailLedger(controller, -rate)
...
Plot.GetTaxDueString = function()
    local timeUntil = TimeUntil(ServerSettings.Plot.Tax.TaxEpoch, ServerSettings.Plot.Tax.TaxInterval)
    if ( timeUntil > ServerSettings.Plot.Tax.TaxInterval:Subtract(ServerSettings.Plot.Tax.TaxingMaxTimeSpent) ) then return "Taxing.." end
    return TimeSpanToWords(timeUntil, false, {Seconds = true})
end

Plot.GetNegativeTaxDueString = function()
    local timeUntil = TimeUntil(ServerSettings.Plot.Tax.SaleEpoch, ServerSettings.Plot.Tax.SaleInterval)
    if ( timeUntil > ServerSettings.Plot.Tax.SaleInterval:Subtract(ServerSettings.Plot.Tax.TaxingMaxTimeSpent) ) then return "Now!" end
    return TimeSpanToWords(timeUntil, false, {Seconds = true})
end
...
Plot.TaxAll = function(clusterController, cb, force)
    ...
    if ( force ~= true ) then
        ...
        if not( IsTime(ServerSettings.Plot.Tax.TaxEpoch, ServerSettings.Plot.Tax.TaxInterval, now, ServerSettings.Plot.Tax.TaxTimeResolution) ) then return cb() end
        -- already taxed this interval
        local lastTax = GlobalVarReadKey("Taxes", "LastTax")
        if ( lastTax ~= nil and now:Subtract(lastTax) <= ServerSettings.Plot.Tax.TaxTimeResolution ) then return cb() end
    end
...
Plot.Sale = function(controller, index, cb)
    ...
    Plot.RemoveOwner(controller, function()
        -- clear balance
        controller:DelObjVar("PlotTaxBalance")
        ...
        local startTime = GetNextTime(ServerSettings.Plot.Auction.StartEpoch, ServerSettings.Plot.Auction.StartInterval)
        index = (index % ServerSettings.Plot.Auction.Slots.Max ) - 1
        if ( index > 0 ) then
            startTime = startTime:AddSeconds(ServerSettings.Plot.Auction.Slots.Interval.TotalSeconds * index)
        end
        ...
        Create.AtLoc("plot_forsale_sign", loc, function(sign)
            ...
            DebugMessage(string.format("[Tax] PlotController(%s) auction starts at %s.", controller.Id, startTime))
            cb()
        end, true)
    end)
end
...
Plot.SaleAll = function(clusterController, cb, force)
    ...
    if ( force ~= true ) then
        ...
        if not( IsTime(ServerSettings.Plot.Tax.SaleEpoch, ServerSettings.Plot.Tax.SaleInterval, now, ServerSettings.Plot.Tax.SaleTimeResolution) ) then return cb() end
        -- already done sale this interval
        local lastSale = GlobalVarReadKey("Taxes", "LastSale")
        if ( lastSale ~= nil and now:Subtract(lastSale) <= ServerSettings.Plot.Tax.SaleTimeResolution ) then return cb() end
    end
...
Plot.CalculateStartingBid = function(controller)
    -- starting bid is the cost of the plot at least.
    return Plot.CalculateCostController(controller)
end
...
Plot.CheckBidRefund = function(playerObj, cb)
    ...
    if ( type(endsAt) == "number" ) then -- legacy support
        if ( os.time(os.date("!*t")) < endsAt ) then return cb() end
    else
    ...
    -- give the refund
    Create.Coins.InContainer(bank, refund, function(coins)
        if ( coins ) then
            -- refund was successful
            playerObj:DelObjVar("BidData")
            playerObj:SystemMessage("Plot auction lost, "..ValueToAmountStr(refund).." has been returned to your bank.", "event")
            Plot.ClearActiveBid(playerObj, userId, cb)
        else
            DebugMessage("Refund Failed for playerId: "..playerObj.Id)
            cb()
        end
    end, true) -- Don't autostack these
end
...
--- Will set the plot's house list to real world houses residing in plot, fixes invalid houses in plot list and similar.
-- @param controller
Plot.CalculateHouses = function(controller)
    if not( controller ) then
        LuaDebugCallStack("[Plot.CalculateHouses] controller not provided.")
        return
    end 

    local houses = FindObjects(SearchMulti({
        SearchObjVar("PlotController", controller, ServerSettings.Plot.SearchSize),
        SearchObjVar("IsHouse", true),
    }), controller)
    
    if ( #houses > 0 ) then
        controller:SetObjVar("PlotHouses", houses)
    else
        controller:DelObjVar("PlotHouses")
    end
end
...
--- Calculate the purchase cost for a size of land
-- @param (array)boundsData
-- @return (number) cost
Plot.CalculateCost = function(boundsData)
    if not( boundsData ) then return 99999 end
    local cost = 0
    for i=1,#boundsData do
        local size = boundsData[i][2]
        cost = cost + math.pow(size.X * size.Z, ServerSettings.Plot.CostCoefficient)
    end
    -- round to nearest silver
    cost = math.floor( math.ceil(cost) / 100 ) * 100
    return cost
end

Plot.CalculateCostController = function(controller)
    if not( controller ) then
        LuaDebugCallStack("[Plot.CalculateCostController] controller not provided.")
        return 999999
    end
    
    local bounds = controller:GetObjVar("PlotBounds")
    return Plot.CalculateCost(bounds)
end
...
Plot.CalculateTaxMax = function(rate)
    return math.floor((rate or ServerSettings.Plot.Tax.MinimumPayment) * ServerSettings.Plot.Tax.MaxBalanceRateModifier)
end
...
Plot.TryAdjust = function(playerObj, controller, amount, i)
    ...
    local o = nil
    Plot.ForeachLockdown(controller, function(object)
        local objectBounds = GetTemplateObjectBounds(object:GetCreationTemplateId()) or {}
        if ( #objectBounds > 0 ) then
            if ( Plot.IsObjectOutOfBounds(object:GetLoc(), object:GetRotation(), objectBounds, {newBound}) ) then
                o = object
                return true
            end
        else
    ...
    if ( o ) then
        playerObj:SystemMessage(o:GetName() .. " is preventing the resize.", "info")
        return false
    end
...
Plot.GetAtLoc = function(loc, buffered)
    if not( loc ) then
    ...
    for i=1,#nearbyPlots do
        local contains, bounds, index = Plot.ContainsLoc(nearbyPlots[i], loc, buffered)
        if ( contains ) then
...
Plot.AlterLockCountBy = function(controller, amount)
    if not(controller) then
        LuaDebugCallStack("[Plot.AlterLockCountBy] ERROR: controller is nil")
        return
    end
...
Plot.TryLockdown = function(playerObj, controller, object)
    ...
    if ( Plot.IsOwner(playerObj, controller) ) then
        ...
        local objectBounds = GetTemplateObjectBounds(template) or {}
        if ( #objectBounds > 0 ) then
            local rot = object:GetRotation()
            if ( Plot.IsObjectOutOfBounds(loc, rot, objectBounds, Plot.GetBounds(controller)) ) then
                playerObj:SystemMessage("That must be completely inside plot to lockdown.", "info")
                return false
            end
            if not( Plot.CheckObjectBounds(playerObj, loc, rot, object, objectBounds, template) ) then
                return false
            end
        else
    ...
    -- if item has object bounds prevent those object bounds from being outside plot
...
Plot.Lockdown = function(playerObj, controller, object)
    ...
    if ( success ) then
        ...
        if ( object:DecayScheduled() ) then object:RemoveDecay() end
        local containedObjects = object:GetContainedObjects()
        for i=1,#containedObjects do
            if ( containedObjects[i]:DecayScheduled() ) then
                containedObjects[i]:RemoveDecay()
            end
        end
...
Plot.Release = function(playerObj, controller, object)
    ...
    if ( success ) then
		...
		local saleItem = FindItemsInContainerRecursive(object,  function(item)
...
--- Given and object and it's rotation and object has objectBounds, determine if any of the corners out of plot bounds.
-- @param loc - Loc()
-- @param rot - Obj:GetRotation()
-- @param objectBounds - GetTemplateObjectBounds()
-- @param plotBounds - Plot.GetBounds()
-- @return true/false
Plot.IsObjectOutOfBounds = function(loc, rot, objectBounds, plotBounds)
    if ( objectBounds ~= nil and #objectBounds > 0 ) then
        local box, point
        for i=1,#plotBounds do
            for ii=1,#objectBounds do
                box = objectBounds[ii]:Flatten():Rotate(rot.Y)
                for iii=1,4 do
                    point = box.Points[iii]
                    point.X = point.X + loc.X
                    point.Z = point.Z + loc.Z
                    if not( plotBounds[i]:Contains(point) ) then
                        return true
                    end
                end
            end
...
--- objects specified as solid objects cannot be moved into other objects specified as solid objects, also prevents them moving up/down
-- @param playerObj
-- @param loc
-- @param rot
-- @param object
-- @param objectBounds
-- @param template
-- return true/false (false if blocked)
Plot.CheckObjectBounds = function(playerObj, loc, rot, object, objectBounds, template)
    if ( SolidObjects[template] ~= nil and objectBounds ~= nil and #objectBounds > 0 ) then
        if ( loc.Y ~= 0 ) then
            playerObj:SystemMessage("Must be flat on ground.", "info")
            return false
        end
        local nearbyObjects = FindObjects(SearchRange(loc, 3), object)
        local templatee, bounds, rott, locc, y
        local b = {}
        for i=1,#nearbyObjects do
            templatee = nearbyObjects[i]:GetCreationTemplateId()
            if ( SolidObjects[templatee] ) then -- solid object only collide with other solid objects, everything else allows awesome deco
                bounds = GetTemplateObjectBounds(templatee) or {}
                if ( #bounds > 0 ) then
                    rott = nearbyObjects[i]:GetRotation()
                    locc = nearbyObjects[i]:GetLoc()
                    for j=1,#bounds do
                        for k=1,#objectBounds do
                            if ( y == nil ) then
                                y = objectBounds[k]:GetMaxY() + loc.Y
                            end
                            if ( loc.Y < (bounds[j]:GetMaxY() + locc.Y) and y > locc.Y ) then
                                if ( b[k] == nil ) then
                                    -- cache this so we only have to adjust it once
                                    b[k] = objectBounds[k]:Flatten():Rotate(rot.Y)
                                    b[k] = b[k]:Add(b[k], loc)
                                end
                                bounds[j] = bounds[j]:Flatten():Rotate(rott.Y)
                                bounds[j] = bounds[j]:Add(bounds[j], locc)
                                if ( b[k]:Intersects(bounds[j]) ) then
                                    playerObj:SystemMessage("Another object is in the way.", "info")
                                    return false
                                end
                            end
                        end
...
Plot.TryUnpack = function(playerObj, packedObj, loc)
    ...
    if not( Plot.IsPassable(loc) ) then				
        playerObj:SystemMessage("Something is in the way.", "info")
        return false
    end
    ...
    -- some items cannot be placed inside
    if ( house and templateData.ObjectVariables and templateData.ObjectVariables.ExteriorDecoration ) then
    ...
    local objectBounds = GetTemplateObjectBounds(template) or {}
    local rot = Loc()
    if ( Plot.IsObjectOutOfBounds(loc, rot, objectBounds, bounds) ) then
        playerObj:SystemMessage("Must be placed completely inside plot.", "info")
        return false
    end
...
Plot.GetHouseBounds = function(houseData, loc, buffered, roofBounds)    
    ...
    for i=1,#bounds do
        ...
        local topLeft,topRight,bottomLeft = bound.TopLeft,bound.TopRight,bound.BottomLeft
...
Plot.DestroyHouse = function(playerObj, house)
    ...
    if ( Plot.RemoveHouse(controller, house) ) then
...
--- Remove the house/foundation from the plot's list of houses, does not destroy the house.
-- @param controller
-- @param house
-- @return true/false
Plot.RemoveHouse = function(controller, house)
    if not( controller ) then
        LuaDebugCallStack("[Plot.RemoveHouse] controller not provided.")
        return false
    end
    if not( house ) then
        LuaDebugCallStack("[Plot.RemoveHouse] house not provided.")
        return false
    end

    -- remove the house from the plot controller's houses
    local houses = controller:GetObjVar("PlotHouses") or {}
    local newHouses = {}
    for i=1,#houses do
        if ( houses[i] ~= house ) then
            newHouses[#newHouses+1] = houses[i]
        end
    end
    controller:SetObjVar("PlotHouses", newHouses)
...
Plot.TryRotateTo = function(playerObj, object, rot)
    if not( playerObj ) then
        LuaDebugCallStack("[Plot.TryRotateTo] playerObj not provided.")
        return false
    end
    if not( object ) then
        LuaDebugCallStack("[Plot.TryRotateTo] object not provided.")
        return false
    end
    if not( rot ) then
        LuaDebugCallStack("[Plot.TryRotateTo] rot not provided.")
        return false
    end
    ...
    if ( success ) then
        if ( house ) then
            ...
            if ( #objectBounds > 0 ) then
                ...
                inbounds = not Plot.IsObjectOutOfBounds(loc, rot, objectBounds, Plot.GetBounds(controller))
            else
                inbounds = Plot.ContainsLoc(controller, loc)
            end
...
Plot.TaxWarn = function(playerObj, save)
    ...
    for i=1,#userPlots do
        Plot.GetInfoRemote(playerObj, userPlots[i], function(info)
            ...
            if ( diff < 0 or info.Balance < 0 ) then
                local warning = ""
                if ( info.Balance < 0 ) then
                    warning = string.format("Warning, in %s your plot '%s' will be [FF0000]taken[-] from you to be [FF0000]Auctioned[-].\n\nA Minimum of %s is required for the plot to remain in your possession. Tax can be paid directly at the plot's mailbox or from any banker.",Plot.GetNegativeTaxDueString(),info.Name,ValueToAmountStr(-info.Balance,false,true))
                else
                    warning = string.format("Warning, in %s your plot '%s' will have a [FF0000]negative[-] balance that could lead to foreclosure.\n\nA Minimum of %s is required for the plot to remain in your possession. Tax can be paid directly at the plot's mailbox or from any banker.",Plot.GetTaxDueString(),info.Name,ValueToAmountStr(-diff,false,true))
                end
...
Plot.ValidateBindLocation = function(playerObj, loc)
    if not( IsPassable(loc) ) then
        return "Something is blocking the location."
    end
    local plot = Plot.GetAtLoc(loc)
    if ( plot ) then
        local bindstones = FindObjects(SearchMulti({SearchRange(loc, 2.2),SearchModule("bindstone")}),GameObj(0))
        if ( #bindstones < 1 ) then
            return "No bindstone found at location."
        end
    end
    if ( plot and not Plot.IsOwner(playerObj, plot) ) then
        local any = false
        Plot.ForeachHouse(plot, function(house)
            any = Plot.IsHouseCoOwner(playerObj, house) or Plot.IsHouseFriend(playerObj, house)
            return any
        end)
        if not( any ) then
            return "Not allowed to use that bindstone."
        end
    end
    return true
end
...
-- calculate cost of land deed based on minimum plot cost
CustomItemValues.land_deed = Plot.CalculateCost({{nil,Loc(ServerSettings.Plot.MinimumSize,0,ServerSettings.Plot.MinimumSize)}})
