...
function CanUseCase(user, targetObj, useCase)
	if(useCase.Restrictions ~= nil) then
		...
		for i,restrictionEntry in pairs(useCase.Restrictions) do
			...
			elseif(restrictionEntry == "IsInBackpack") then
				if not( IsInBackpack(targetObj, user) ) then
					return false
				end
			...
			elseif(restrictionEntry == "HasPlotControl") then
				if ( IsGod(user) and not TestMortal(user) ) then return true end
				local topCont = targetObj:TopmostContainer() or targetObj
				local plot
		...
		if ( targetObj:IsMobile() ) then
			...
			if ( IsPlayerCharacter(targetObj) ) then
				...
				if ( targetObj ~= user ) then
					...
					if not (IsInFriendList(user, targetObj)) then
						table.insert(menuItems,"Send Friend Request")
					end
				...
				if(isDead) then					
					...
					if (IsController(user,targetObj) and not targetObj:HasObjVar("merchantOwner") ) then						
						if (( IsGod(user) and not TestMortal(user) ) or (IsMount(targetObj) and GetPetSlots(user) > ServerSettings.Pets.MaxSlotsToAllowDismiss)) then
			...
			if not(isDead) then
				...
				elseif ( IsPlayerCharacter(targetObj) or IsPet(targetObj) or (IsHiredMerchant(user,targetObj)) or IsDemiGod(user) ) then
