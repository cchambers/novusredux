MobileEffectLibrary.Focus = 
{
	...
	OnEnterState = function(self,root,target,args)
		...
		if (chest ~= nil) then
			if (GetArmorClassFromType(GetArmorType(chest)) == "Heavy") then 
				self.ParentObj:SystemMessage("You cannot meditate in heavy armor.", "info")
				return EndMobileEffect(root)
			end
		end

		if (legs ~= nil) then
			if (GetArmorClassFromType(GetArmorType(legs)) == "Heavy") then 
				self.ParentObj:SystemMessage("You cannot meditate in heavy armor.", "info")
				return EndMobileEffect(root)
			end
		end
		...
		if (head ~= nil) then
			if (GetArmorClassFromType(GetArmorType(head)) == "Heavy") then 
				self.ParentObj:SystemMessage("You cannot meditate in heavy armor.", "info")
				return EndMobileEffect(root)
			end
		end
