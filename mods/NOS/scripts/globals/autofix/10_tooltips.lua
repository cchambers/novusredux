...
AutoFixes[index] = {
    DoFix = function(object)
        ...
        if ( object:GetCreationTemplateId() == "ancient_relic" ) then
            AutoFixReplaceItem(object, "ancient_relic", function(item)
                if ( item ) then
                    relicsReplaced = relicsReplaced + 1
                end
            end)
        end
