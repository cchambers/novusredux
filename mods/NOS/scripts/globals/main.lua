...
function IsDead(target)
	...
	return target:HasObjVar("IsDead") or target:GetSharedObjectProperty("IsDead") == true
end
...
-- this runs until cb returns falsely
function ForEachItemInContainerRecursive(container, cb, depth)
	if not( container:IsContainer() ) then return end
	depth = depth and (depth + 1) or 1
	local objs = container:GetContainedObjects()
	for i=1,#objs do
		if not( cb(objs[i], depth) ) then return end
		ForEachItemInContainerRecursive(objs[i], cb, depth)
	end
end
