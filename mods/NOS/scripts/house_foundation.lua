...
RegisterEventHandler(EventType.Message, "UseObject", function(user,usedType)
    ...
    elseif (usedType == "Destroy Foundation") then
        if ( Plot.IsOwner(user, m_Controller) ) then
            local message = "Are you sure you wish to destroy this foundation?\n\n[FF0000]No resources will be refunded.[-]"
            local template = this:GetObjVar("Blueprint")
            if ( template ) then
                message = message .. "\n\nYou will be given a new blueprint."
            end
            -- Are you sure?
            ClientDialog.Show{
                TargetUser = user,
                ...
                ResponseFunc = function( userr, buttonId )
                    if ( userr == user ) then
                        ...
                        if ( buttonId == 0 ) then
                            if ( template ) then
                                Create.InBackpack(template, user, nil, function(blueprint)
                                    if ( blueprint ) then
                                        Plot.RemoveHouse(m_Controller, this)
                                        this:Destroy()
                                    else
                                        user:SystemMessage("Failed to create the blueprint.", "info")
                                    end
                                end)
                            else
                                Plot.RemoveHouse(m_Controller, this)
...
RegisterSingleEventHandler(EventType.ModuleAttached, "house_foundation", function()
    ...
    AddUseCase(this,"Destroy Foundation",false)
