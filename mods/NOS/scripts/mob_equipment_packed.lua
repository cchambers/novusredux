...
RegisterEventHandler(EventType.ClientTargetGameObjResponse, "targetObj", 
	function(targetObj,user)
		...
		-- checks if it already has a pack
		if ( targetObj:HasObjVar("HasPetPack") ) then
			user:SystemMessage("That pet already has a pack!","info")
			return
		end
		...
		if( unpackedTemplate ~= nil ) then
			ClientDialog.Show{
				...
				ResponseFunc = function (user,buttonId)
			    		if(buttonId == 0 and targetObj and targetObj:IsValid() and IsController(user,targetObj)) then
							local TargetBackpack = (targetObj:GetEquippedObject("Backpack"))
							if (TargetBackpack) then
								TargetBackpack:Destroy()
							end
