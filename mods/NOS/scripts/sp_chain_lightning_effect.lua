...
local hasHit = {}
local rangeSkill = GetSkillLevel(this, "MagicAffinity")
...
RegisterEventHandler(
	EventType.Message,
	"SpellHitUserEffectsp_chain_lightning_effect",
	function(target)
		hasHit[target] = true
		local potJumps = 0
...
function ProcessPhase(phaseNum)
	...
	for i, v in pairs(targQueue[phaseNum]) do
		...
		if (hasHit[i] == nil) then
			this:SendMessage("RequestMagicalAttack", "Chainlightning", i, this, true)
		end
	end
	if (curPhase <= lastPhase) then
		this:ScheduleTimerDelay(TimeSpan.FromSeconds(1), "RemoveChainLightningTimer")
	end
end
