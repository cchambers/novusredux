...
RegisterEventHandler(EventType.DynamicWindowResponse,"PlayerAchievementWindow",
	function (user,buttonId)
		...
		if (buttonId == "LockTitle") then
			local lockTitle = user:HasObjVar("LockTitle")
			if (lockTitle) then
				user:DelObjVar("LockTitle")
			else
				user:SetObjVar("LockTitle",true)
			end
			ReopenAchievementWindow()
			return
		end
		...
		if (buttonId == "RemoveTitle") then
			if not (user:HasObjVar("LockTitle")) then
				user:SystemMessage("Title has been removed.","info")
				user:SetSharedObjectProperty("Title","")
				RemoveTooltipEntry(user,"Title")
				ReopenAchievementWindow()
			else
				user:SystemMessage("Disable title lock to change the title.","info")
				return
			end
			return
		end
