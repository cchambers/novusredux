...
function ShouldSpawn(spawnInfo,spawnIndex)  
    ...
    --DebugMessage(this,"ShouldSpawn "..spawnInfo.TemplateId.." isValid: "..tostring(isValid).." isDead: "..tostring(isDead))
    ...
    if( isValid and not(isDead) and not(IsPet(spawnData[spawnIndex].ObjRef)) ) then
        spawnData[spawnIndex].GoneTime = nil
        return false
    end
    ...
    --DebugMessage(this,"Checking spawnDelay "..tostring(spawnDelay.TotalSeconds))
    if(spawnDelay.TotalSeconds ~= 0) then
        -- this mob just disappeared or died
        local goneTime = spawnData[spawnIndex].GoneTime
        --DebugMessage(this,"spawnData[spawnIndex].ObjRef: "..tostring(spawnData[spawnIndex].ObjRef).. ", goneTime: "..tostring(goneTime) )
        if( spawnData[spawnIndex].ObjRef and not(goneTime) ) then  
            --DebugMessage(this,"MOB DEAD OR DESPAWNED")   
        ...
        if(goneTime and DateTime.UtcNow < (goneTime + spawnDelay)) then
            --DebugMessage(this,"Waiting on spawn timer")   
            return false
        end
    end
    ...
    if (spawnInfo.SpawnTime ~= nil and not(WithinSpawnTime(spawnInfo))) then
        --DebugMessage(this,"Waiting on spawn time (time of day)")   
        return false
    end

    -- roll to spawn
    local chanceRoll = math.random()*100
    --DebugMessage(this,"Checking on chanceRoll "..tostring(chanceRoll).." against "..tostring(spawnInfo.Chance))
    if (spawnInfo.Chance ~= nil and chanceRoll > spawnInfo.Chance) then
        -- reset gone time if spawn failed on chance
        if(spawnDelay.TotalSeconds ~= 0) then
            --DebugMessage(this,"Chance failed, reset gonetime")
            spawnData[spawnIndex].GoneTime = DateTime.UtcNow
        end
        return false
    end
...
function CheckSpawn(info)
    ...
    if(#spawnQueue == 0) then
        ...
        if not(this:HasTimer("spawn_queue_timer")) then
...
RegisterEventHandler(EventType.CreatedObject, "created", 
    function(success,objRef,callbackData)
        ...
        if success then
            ...
            local thisSpawnInfo = spawnInfo[callbackData.InfoIndex]
            --DebugMessage("--SC CREATED ",tostring(callbackData.InfoIndex),tostring(callbackData.SpawnIndex),tostring(objRef))
            thisSpawnInfo.SpawnData[callbackData.SpawnIndex].ObjRef = objRef
            thisSpawnInfo.SpawnData[callbackData.SpawnIndex].GoneTime = nil
