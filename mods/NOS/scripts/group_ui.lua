...
function KarmaConsent()
    ClientDialog.Show{
        ...
        TitleStr = "Consent",
        DescStr = "\n\n[FF8C00]WARNING: All consenting members of this group can perform negative karma actions without consequence. This includes, looting, attacking, killing, and anything guard protected.[-]",
        Button1Str = "Consent",
        Button2Str = "Reject",
        ResponseObj = this,
        ResponseFunc = function(user,buttonId)
            local buttonId = tonumber(buttonId)
            DebugMessage("MADE IT TO HERE...")
            if ( user == nil or buttonId == nil ) then return end

            if ( buttonId == 0 ) then
                DebugMessage("AND TO HERE...")
