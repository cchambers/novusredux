...
RegisterEventHandler(EventType.ModuleAttached, GetCurrentModule(), 
	function()
		...
		if (mContinuousSpawns) then
			this:ScheduleTimerDelay(TimeSpan.FromSeconds(SPAWN_TIME),"ContinuousSpawn")
		end
	end)

RegisterEventHandler(EventType.Timer, "ContinuousSpawn", 
	function()
		CheckSpawns()
		this:ScheduleTimerDelay(TimeSpan.FromSeconds(SPAWN_TIME), "ContinuousSpawn")
	end)
...
OverrideEventHandler("destroyable_object", EventType.Message, "DamageInflicted", 
	function(damager,damageAmt)
		...
		--If pet is attacking mob spawner, aggro on pet owner
		if (IsPet(damager)) then
			mCurTarget = damager:GetObjVar("controller") or damager
		else
			mCurTarget = damager
		end
...
RegisterEventHandler(EventType.CreatedObject,"mob_spawned",
	function(success,objRef)
		...
		if (mCurTarget ~= nil) then
			objRef:SendMessage("AddThreat",mCurTarget,10)
