...
function CheckSpawn()
    ...
    local spawnDay = this:GetObjVar("spawnDay")
    if (spawnDay) then
    ...
    if( templateId == nil ) then
        DebugMessage("ERROR: Simple mob spawner picked invalid template: "..tostring(this.Id).. " SpawnerTemplate: "..tostring(this:GetCreationTemplateId()))
        return
    end
