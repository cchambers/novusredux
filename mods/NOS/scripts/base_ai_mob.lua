...
function RunToTarget(clearPath)
   ...
   -- end
    ...
    if( target ~= nil ) then
       ...
       -- DebugMessage("Getting here.")
        ...
        local speed = AI.GetSetting("ChargeSpeed")

        -- update run speed based on HP.
        if ( GetCurHealth(this) < ( GetMaxHealth(this) * 0.35 ) ) then -- less than half health
            speed = speed * 0.5
        end

        speed = math.max(((speed + GetMobileMod(MobileMod.MoveSpeedPlus))) * GetMobileMod(MobileMod.MoveSpeedTimes, 1), 0.5)

        if ( clearPath ) then this:ClearPathTarget() end
        ...
        this:PathToTarget(target,pathDistance,speed)
    else
