...
function InterruptTrading()
	if (mTradeInterrupted == false) then
...
function CleanUp()
	...
	DelView("CheckBagsForNewItemsA")
	DelView("CheckBagsForNewItemsB")
	...
	if (mTraderB ~= nil and mTraderB:IsValid()) then
		...
		mTraderB:DelModule("trading_target_controller")
	end
		...
		ClientDialog.Show{
		    ...
		    ResponseFunc= function(user,buttonId)
				...
				elseif (buttonId == 0 and ValidateTradeTarget(target) and CanTradeWith(this,target)) then
					...
					mTraderB:AddModule("trading_target_controller",{TradeTarget=mTraderA})
...
RegisterEventHandler(EventType.Message,"ClearTradeAccept",
	function()
		CancelAccept()
		ShowTradingWindow(mTraderA)
		ShowTradingWindow(mTraderB)
...
RegisterEventHandler(EventType.CreatedObject,"created_trading_pouch",function (success,objRef,user)
	...
	if (success) then
		...
		if(pendingTradeCreates == 0) then				
			...
			local searcherA = SearchContainer(mTradingPouchB)
