...
Guild.PromoteMember = function(member, g, level, force)
	if not( GlobalVarReadKey("User.Online", member) ) then
		this:SystemMessage("[00FF00][Guild] Target member must be online to promote.","custom")
		return false
	end
...
Guild.DemoteMember = function(member, g)
	if not( GlobalVarReadKey("User.Online", member) ) then
		this:SystemMessage("[00FF00][Guild] Target member must be online to demote.","custom")
		return false
	end
...
Guild.CanChangeGuildMaster = function(member, g)
	if not( GlobalVarReadKey("User.Online", member) ) then
		this:SystemMessage("[00FF00][Guild] Target member must be online to transfer guild.","custom")
		return false
	end
	g = g or GuildHelpers.Get(this)
...
Guild.CanKickMembers = function(m,target,g)
	...
	local mylevel = GuildHelpers.GetAccessLevel(m,g)
	local theirlevel = GuildHelpers.GetAccessLevel(target,g)

	if (mylevel == "Guildmaster" and theirlevel =="Guildmaster") then return false end
	if (mylevel == "Officer" and theirlevel =="Guildmaster") then return false end
	if (mylevel == "Officer" and theirlevel =="Officer") then return false end

	if (mylevel == "Member") then return false end
	if (mylevel == "Emissary") then return false end
