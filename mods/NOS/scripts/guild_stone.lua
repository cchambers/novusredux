...
function createGuild(user)
	...
	TextFieldDialog.Show {
		...
		ButtonStr = "Submit",
		ResponseFunc = function(u, newName) 
			DebugMessage("User clicked stone: " .. u.Id)
			-- user:SystemMessage("Your guild name sucks " .. newName)
			...
			-- 		user:SystemMessage("The guild name must longer than 3 characters.")
			-- 		return
			-- 	end

			-- 	if (string.len(newName) > 35) then
			-- 		user:SystemMessage("The guild name must be less than 36 characters.")
			-- 		return
			-- 	end

			-- 	if (#newName:gsub("[%a ]", "") ~= 0) then
			-- 		user:SystemMessage("[$1696]")
			-- 		return
			-- 	end

			-- 	if (ServerSettings.Misc.EnforceBadWordFilter and HasBadWords(newName)) then
			-- 		user:SystemMessage("[$1697]")
			-- 		return
			-- 	end

			-- 	if (g ~= nil) then
			-- 		user:SystemMessage("You are already in a guild")
			-- 		return
			-- 	end

			-- 	local guildInvite = user:GetObjVar("GuildInvitation")
...
function createGuildTag(user, newName)
	TextFieldDialog.Show {
		TargetUser = user,
		Title = "Guild Tag",
		DialogId = "Create Guild",
		Description = "[$1698]",
		ResponseFunc = function(tag)
			if (tag ~= nil and tag ~= "") then
				...
				if (string.len(tag) < 2) then
					user:SystemMessage("The guild tag must be more than 1 character.")
					return
				end

				if (string.len(tag) > 4) then
					user:SystemMessage("The guild tag must be less than 4 characters.")
					return
				end

				if (tag:match("%A")) then
					user:SystemMessage("The guild tag can only contain letters.")
					return
				end

				if (ServerSettings.Misc.EnforceBadWordFilter and HasBadWords(tag)) then
					user:SystemMessage("[$1699]")
					return
				end
				...
				Guild.TryCreate(user, newName, tag)
			end
		end
	}
end
	...
	function(user, usedType)
		...
		if (mCurrentTab == "Roster") then
			...
			for i, memberData in pairs(GetRosterList(g)) do
				...
				if (isSelected and user.Id ~= mSelectedMemberId) then
					local buttons = {}
					...
					if (Guild.CanKickMembers(user)) then
						table.insert(buttons, "Kick")
					end

					if (Guild.CanBePromotedBy(user, GameObj(memberData.Id))) then
						table.insert(buttons, "Promote")
						table.insert(buttons, "Demote")
					end

					if (Guild.GetAccessLevel(user, Guild.Get(user)) == "Guildmaster") then
		...
		elseif (mCurrentTab == "Guild") then
			...
			local guildInfo = Guild.GetGuildMessage(user, g, "Info")
			...
			local guildMessage = Guild.GetGuildMessage(user, g, "MOTD")
			newWindow:AddLabel(25, 37 + 180 + 40, guildMessage, 500, 200, 18)

			if (Guild.HasAccessLevel(user, "Officer", g)) then
		...
		user:SystemMessage("ME")
		local membersOnline = Guild.GetOnlineMemberCount(user, g)
