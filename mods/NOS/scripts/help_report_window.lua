...
function ShowHelpReportWindow()
	...
	newWindow:AddButton(x+20, 70, "Type|harassment", "I'm reporting another player.", 100, 23, "I wish to target another player for this report.", "", false,"Selection",GetButtonState(helpWindowType,"harassment"))

	if(helpWindowType=="harassment") then
		newWindow:AddLabel(x+80,130,"I'm reporting: ",0,0,19)
...
function HandleHarassmentTarget (target, user)
	...
	if(target and target ~= this and target:IsPlayer()) then
		harassmentTarget = { Name=target:GetName(), Id=target.Id, UserId=target:GetAttachedUserId() }
	else
...
function BuildReportData()
	return json.encode({
		...
		Cluster = ServerSettings.ClusterId,
		UserID = this:GetAttachedUserId(),
		CharacterId = tostring(this.Id),
		...
		HarassmentUserId = harassmentTarget and harassmentTarget.UserId
	})
end
