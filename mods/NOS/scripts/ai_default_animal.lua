...
-- Combat AI settings
AI.Settings.AggroChanceAnimals = 5 --1 out of this number chance to attack other animals
...
AI.Settings.ChanceToNotAttackOnAlert = 2 --1 out of this number chance to attack when alerted
...
function IsFriend(target)
    ...
    local hostileLevel = this:GetObjVar("HostileLevel") or 0
    local targetHostileLevel = target:GetObjVar("HostileLevel") or 0

    if (hostileLevel > targetHostileLevel) then
        local myTeamType = this:GetObjVar("MobileTeamType")
        local targetTeamType = target:GetObjVar("MobileTeamType")
        local randomChance = math.random(AI.GetSetting("AggroChanceAnimals"))

        if (myTeamType ~= targetTeamType and randomChance == 1) then
            return false
        end
    end
