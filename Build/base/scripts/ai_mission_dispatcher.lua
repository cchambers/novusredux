...
function Dialog.OpenListMissionsDialog(user)
	...
	if (#userMissions >= MAX_MISSIONS) then
		...
		if (mSelectedMissions ~= nil) then
		    for i, j in pairs(mSelectedMissions) do
		    	local missionTable = GetMissionTable(mSelectedMissions[i].Key)
		    	local descriptionText = missionTable.Title
		    	response[responseIndex] = {}
			    response[responseIndex].text = tostring(descriptionText).." at "..GetRegionalName(mSelectedMissions[i].Loc) or ServerSettings.SubregionName or ServerSettings.WorldName
...
function OpenMissionDetailsDialog(user, selectedMissionIndex)
	local missionTable = GetMissionTable(mSelectedMissions[selectedMissionIndex].Key)
	local descriptionText = missionTable.Description
	local difficultyText = GetDifficultyDescription(user, missionTable.Difficulty)
	local directionDescription = GetDirectionDescription(mSelectedMissions[selectedMissionIndex].Loc) or ""
...
OverrideEventHandler("base_ai_npc", EventType.DynamicWindowResponse,"Responses", 
	function (user,buttonId)
		...
		elseif(buttonId:match("Accept")) then
			...
			AddMission(user, mSelectedMissions[missionIndex].Key, mSelectedMissions[missionIndex].Loc)
...
function GetUserDifficulty(user)
	...
	local evocationLevel = GetSkillLevel(user, "EvocationSkill")

	local score = 1
	if (weaponLevel > evocationLevel) then
		score = (weaponLevel + GetSkillLevel(user, "MeleeSkill"))/2
	else
		score = (evocationLevel + GetSkillLevel(user, "MagicAffinitySkill"))/2
	end
