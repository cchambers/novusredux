...
local controller,plotBounds,newPlotBounds,size,newSize,rate,newRate,amount,step,canAdjust
...
function UpdateWindow()
    ...
    if(can) then
        ...
        dynamicWindow:AddLabel(162,84,"Commit Cost: ".. (cost > 0 and GetAmountStrShort(ValueToAmounts(cost)) or "0"),0,0,18)        
    end
    ...
    ClientDialog.Show{
        ...
        ResponseFunc = function( user, buttonId )
            ...
            if ( user == this and buttonId == 0 ) then
                if ( cost < 1 or ConsumeResourceContainer(this, "coins", cost) ) then
