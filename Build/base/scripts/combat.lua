		...
		if ( hitItem ~= nil ) then
			...
			if ( isPlayer ) then
				...
				if ( (armorClass == "Light" or armorClass == "Heavy") and ValidCombatGainTarget(damageInfo.Attacker,victim) ) then																	
					CheckSkill(victim, string.format("%sArmorSkill", armorClass), GetSkillLevel(damageInfo.Attacker, EquipmentStats.BaseWeaponClass[_Weapon.RightHand.Class].WeaponSkill))					
				end
			end
		end
	...
	if ( mCurrentTarget ~= newTarget ) then
		...
		if(this:GetObjVar("AutotargetEnabled")) then
			if(mCurrentTarget == nil) then
				...
				ShowStatusElement(mCurrentTarget,{User=this,DialogId="TargetWindow",ScreenX=200,ScreenY=20,FrameImage="UtilityBar_SmallRedFrame"})
			end
		end
	end
...
function SetupView(hand)
	...
	if ( mCurrentTarget == nil or mCurrentTarget == this or not InCombat(this) ) then return end
	if ( not _Weapon[hand].Class or _Weapon[hand].ShieldType ) then return end
...
RegisterEventHandler(EventType.EnterView, "RightHandAttackRange", function(obj)
	Verbose("Combat", "RightHandAttackRange", obj)
	if ( not mInCombatState or obj ~= mCurrentTarget ) then return end
	PerformWeaponAttack(mCurrentTarget, "RightHand")
end)

--Enter attack range of left hand weapon
RegisterEventHandler(EventType.EnterView, "LeftHandAttackRange", function(obj)
...
RegisterEventHandler(EventType.Timer, "SWING_TIMER_RightHand", function()
	mSwingReady.RightHand = true
	PerformWeaponAttack(mCurrentTarget, "RightHand")
end)

--Left Hand Swing Timer for Dual wield support
RegisterEventHandler(EventType.Timer, "SWING_TIMER_LeftHand", function()
...
RegisterEventHandler(EventType.Message, "DelaySwingTimer", function (delay, hand)
	if not( InCombat(this) ) then return end
...
function DrawBow()
	...
	if ( 
		not _Weapon.RightHand.IsRanged
		or
		mBowDrawn
		or
		mIsMoving
...
RegisterEventHandler(EventType.Timer, "DoRecalculateStats", function()
