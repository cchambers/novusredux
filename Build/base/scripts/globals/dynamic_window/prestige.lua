function PrestigeAbilityWindow(playerObj, prestigeClass, abilityName, slot)

    if ( playerObj and prestigeClass and abilityName and slot ) then
        playerObj:SystemMessage("set ability: "..prestigeClass.."."..abilityName.." to slot "..slot)
        GiveMobileMinimumSkillXpForAbility(playerObj, abilityName)
    ...
    if ( prestigeClass ) then
        if ( abilityName ) then
            dynamicWindow:AddLabel(
                ...
                PrestigeData[prestigeClass].Abilities[abilityName].Action.DisplayName, --(string) text in the label
            ...
            dynamicWindow:AddButton(
                ...
                "makebook "..abilityName, --(string) server command to send on button click (default to none)
            ...
            for s=1,3 do
                dynamicWindow:AddButton(
                    ...
                    buttonHeight,--(number) height of the button (default decided by type of button)
                    PrestigeData[prestigeClass].Abilities[abilityName].Action.Description, --(string) mouseover tooltip for the button (default blank)
                    "prestige "..prestigeClass.." "..abilityName.." "..s, --(string) server command to send on button click (default to none)
                    true,--closeWindowOnClick, --(boolean) should the window close when this button is clicked? (default true)
                    "List"--buttonType, --(string) button type (default "Default")
                    --buttonState, --(string) button state (optional, valid options are default,pressed,disabled)
                    --customSprites --(table) Table of custom button sprites (normal, hover, pressed. disabled)
...
function PrestigeCommandMakeBook(playerObj, prestigeAbility)
    if not ( prestigeAbility ) then
        playerObj:SystemMessage("Please specify a prestige ability.","info")
        return
    end

    GiveMobileMinimumSkillXpForAbility(playerObj, prestigeAbility)

    RegisterSingleEventHandler(EventType.CreatedObject,"book_created",function (success,objRef)
        objRef:SendMessage("SetBook",prestigeAbility)
    end)
