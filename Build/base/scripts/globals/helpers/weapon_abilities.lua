...
function GetWeaponAbility(mobileObj, primary)
	...
	if ( mobileObj ) then
		-- first check for dynamic WeaponAbilities (npcs)
...
function QueueWeaponAbility(mobileObj, primary, weaponAbility)
	if( IsDead(mobileObj) or mobileObj:HasTimer("RecentAbilityUsed") ) then return end
	local primarySecondary = primary and "Primary" or "Secondary"
	...
	if ( weaponAbility == nil ) then
		...
		if ( weaponAbility.NoMount == true ) then
			local mountObj = GetMount(mobileObj)
			if ( mountObj ) then
...
function PlayerUseWeaponAbility(playerObj, primarySecondary)
	if( playerObj == nil or IsDead(playerObj) ) then return end
