...
-- @param mobileObj
-- @return true/false
function ShareGroupCheckPet(playerObj, mobileObj)
    Verbose("Group", "ShareGroupCheckPet", playerObj, mobileObj)
	-- reassign mobileObj to the owner if mobileObj is a pet
    if ( IsPet(mobileObj) ) then mobileObj = mobileObj:GetObjectOwner() or mobileObj end
...
function GroupInfo(member, message, updateUi)
    ...
    member:SystemMessage(string.format("[00FFFF]%s[-]", message), "info")
    -- if the group ui is open, update it.
    if ( updateUi == true and member:HasModule("group_ui") ) then
        member:SendMessage("GroupUpdate")
    end
end